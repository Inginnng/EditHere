...
QString launchAtLoginNotice() {
    WindowsRunValueStore store;
    QString command;
    if (store.read(&command, nullptr) && !command.isEmpty() &&
        command.compare(formattedCommand(QCoreApplication::applicationFilePath()), Qt::CaseInsensitive) != 0)
        return "开机自启项指向其他位置或使用旧的启动参数。关闭并保存后重新开启，可更新为当前应用。";
    return {};
}
