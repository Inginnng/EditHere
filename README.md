...
产品版本唯一来源为 `CMakeLists.txt` 的 `project(... VERSION ...)`。运行时版本和 Mac 应用信息自动使用该值；成功链接后生成 `build/version.txt`（Mac 为 `build-macos/version.txt`），两平台打包脚本据此命名并随包附带 `version.txt`。修改版本后必须重新构建，避免将旧程序标记为新版本。产品版本与 JSON 格式独立管理：`0.8.5` 的精简反馈由 `feedback-v0.7.schema.json` 定义，完整项目使用 `project-v3.schema.json`。

## 运行

Windows 便携包：`dist/HelpDesign-0.8.5-win-x64.zip`。先从托盘退出旧版，再完整解压并运行 `HelpDesign.exe`。请保留同目录 DLL 和插件文件夹；无需安装 Qt、Python、Node 或 .NET。
...
- 批注栏从一开始就固定预留。点编号或移动箭头，在右侧直接编辑，离开输入框自动保存；右上角加号可添加全局意见。框选先建立手动区域，点击区域可添加批注。
...
大爆炸开启时，右侧始终保留批注；点标注工具可在结果上添加意见，编号和移动箭头可直接点击编辑。智能工具用于选块和调整，框选用于补充手动区域；选中组件后可从右侧添加批注。附着在组件上的批注随组件移动、缩放，编辑结果和批注使用同一套画布坐标。
...
构建脚本运行检测、核心数据、布局、界面、画布交互、设置、更新和 Windows 平台八组测试。导出样本及窗口渲染截图存于 `artifacts/native-ui/`。安装 `jsonschema==4.26.0` 后，可运行 `python scripts/validate-exports.py` 独立校验导出。打包只写新目录；重复打包请传入新的 `-OutputDirectory`。
