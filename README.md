...
右键任务栏右下角的 HelpDesign 托盘图标，选择“设置”。窗口包含“快捷键”“外观”“默认行为”和“关于与更新”。点击“保存”应用设置；快捷键和外观立即生效，默认工具/缩放用于下一张图片，启动行为用于下次启动。“取消”不改变当前配置。
...
大爆炸开启时，也可直接选择智能、点或框工具，在当前调整后的画面上添加批注；点击“移动组件”返回组件操作。附着在组件上的批注随组件移动、缩放，编辑结果和批注使用同一套画布坐标。
...
复制和保存采用同一种结构。导出窗口默认勾选包含原图；需要单独发送图片时，可取消勾选以缩短 JSON。Ctrl+S 始终保存内嵌原图的 JSON，单个文件即可重新打开。

| 字段 | 含义 |
| --- | --- |
| `image` | 可选的原图 PNG data URL：`data:image/png;base64,...`，保存的是调整前的完整截图 |
| `annotationSpace` | 固定为 `"result"`，表示批注坐标对应当前调整后的画面 |
| `annotations` | 每条只包含 `point` 或 `rectangle`，以及批注文字 `text` |
