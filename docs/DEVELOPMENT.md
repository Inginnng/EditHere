...
脚本生成通用 `.app` 和本地测试用 `.dmg`，使用临时签名。CLI 一同部署到 `EditHere.app/Contents/MacOS/edithere-cli`；DMG 提供 Applications 拖拽入口。私有仓库的 [macOS 构建记录](https://github.com/Inginnng/EditHere/actions/workflows/native-macos.yml) 保留对应构建的测试结果和 DMG；屏幕录制授权、辅助功能授权及多屏截图仍需实机验收。
